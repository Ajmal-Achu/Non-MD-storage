...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({ pattern: 'ronalod ?(.*)', fromMe: false,dontAddCommandList: true}, (async (message, match) => {

        if (match[1] === '') return await message.sendMessage(need);

        var ttinullimage = await axios.get(`https://api.lolhuman.xyz/api/ephoto2/juventusshirt?apikey=queenamdipublic&text1=$&text2=7{encodeURIComponent(match[1])}`, { responseType: 'arraybuffer' })
